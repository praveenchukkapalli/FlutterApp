...
class MemezSupabaseUser extends BaseAuthUser {
  MemezSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> memezSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = MemezSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
